...
async def main():
    ...
    p2sh_address = address_from_script_public_key(
        script.create_pay_to_script_hash_script(), 'testnet')
    ...
    commit_txs = create_transactions(
        ...
        outputs=[{'address': p2sh_address.to_string(), 'amount':  1 *
                  100_000_000}],
    ...
    for transaction in reveal_txs['transactions']:
        ...
        if commit_output is not None:
            sig = transaction.create_input_signature(
                commit_output, private_key)
            transaction.fill_input(
                commit_output, script.encode_pay_to_script_hash_signature_script(sig))
